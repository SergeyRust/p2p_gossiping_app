...
//     try_handshake(&mut stream).await
// }
...
pub(crate) async fn request_peers(
    ...
    let cmd_byte = u8::from(Message::PeersRequest);
    let cmd_buf = [cmd_byte; 1];
    write_async(write, &cmd_buf).await?;
    let mut len_buf = [0; 4];
    read_async(read, &mut len_buf).await?;
    let len = u32::from_be_bytes(len_buf);
    let mut data_buf = vec![0; len as _];
    read_async(read, &mut data_buf).await?;
    // let peers = deserialize_data(&data_buf)?;
    return if let Ok(peers) = deserialize_data(&data_buf) {
        Ok(peers)
    } else {
...
pub(crate) async fn respond_peers(
    write: &OwnedWriteHalf,
    peers: &HashSet<SocketAddr>)
    -> io::Result<()> {
    let data_buf = serialize_data(peers);
    let data_buf_len = (data_buf.len() as u32).to_be_bytes();
    write_async(write, &data_buf_len).await?;
    write_async(write, &data_buf).await?;
    Ok(())
}
...
pub(crate) async fn write_async(write: &OwnedWriteHalf, buf: &[u8]) -> io::Result<()> {
    let mut written = 0;
    while written < buf.len() {
        write.writable().await?;
        match write.try_write(&buf[written..]) {
            Ok(0) => break,
            Ok(n) => {
                written += n;
            }
...
pub(crate) async fn read_async(read: &OwnedReadHalf, buf: &mut [u8]) -> io::Result<()> {
    let mut red = 0;
    while red < buf.len() {
        read.readable().await?;
        match read.try_read(&mut buf[red..]) {
            Ok(0) => break,
            Ok(n) => {
                red += n;
            }
...
// pub(crate) async fn try_handshake(stream: Arc<TcpStream>) -> ConnectResult<bool> {
//     write_all_async(&*stream, b"req").await?;
//     let mut buf = [0; 4];
...
/// Peer shares his address during the handshake
pub(crate) async fn try_handshake(conn: Arc<Connection>) -> ConnectResult<bool> {
    write_async(&conn.write, b"req").await?;
    let mut buf = [0; 4];
    read_async(&conn.read, &mut buf).await?;
    if &buf != b"resp" {
        let msg = format!("try_handshake() received: {:?}", buf);
        return Err(ConnectError::BadHandshake(msg));
    }
...
async fn accept_handshake(conn: Arc<Connection>) -> ConnectResult<SocketAddr> {
    let mut buf = [0; 3];
    read_async(&conn.read, &mut buf).await?;
    if &buf != b"req" {
        let msg = format!("accept_handshake() received: {:?}", buf);
    ...
    write_async(&conn.write, b"resp").await?;
    Ok(conn.peer_addr)
}
